...
if [ -z "$(git status --porcelain --untracked-files=no)" ] || [ "$1" == "-f" ]; then
  ...
  autopep8 --in-place --select "$errors" -a --recursive domdf_wxpython_tools/
  autopep8 --in-place --select "$belligerent" -a -a -a -a -a --recursive domdf_wxpython_tools/
  >&2 flake8 --select "$errors$belligerent" domdf_wxpython_tools/
